...
then create the interpreter object based on the input model which loads it on the delegate.
```python copy
...
and add a dimension after the last axis to make it `(1, 28, 28, 1)`
```python copy
...
here is an example running on the Khadas Edge2,
```shell
Info: ArmNN v33.0.0
